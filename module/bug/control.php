 ...
 * @package     bug
 * @version     $Id: control.php 3315 2012-07-03 03:11:44Z wwccss $
...
class bug extends control
{
    ...
    public function create($productID, $extras = '')
    {
        ...
        if(!empty($_POST))
        {
            ...
            if($buildID != '' && $buildID != 'trunk'){
                $this->loadModel('build');
                $build = $this->build->getById($buildID);
                $bugs = array();
                if($build->bugs){
                    $bugs = explode(",",$build->bugs);
                    if(!in_array($bugID,$bugs)){
                        $bugs[] = $bugID;
                    }
                }
                else{
                ...
                $bugs_str = join(",",$bugs);
                $this->dao->update(TABLE_BUILD)->set('bugs')->eq($bugs_str)->where('id')->eq($buildID)->exec();
    ...
    public function alphaResolved($bugID)
    {
        ...
        if(!empty($_POST))
        {
            $this->bug->alphaResolved($bugID);
            if(dao::isError()) die(js::error(dao::getError()));
            $actionID = $this->action->create('bug', $bugID, 'alphaResolved', $this->post->comment);
            $this->sendmail($bugID, $actionID);
            die(js::locate($this->createLink('bug', 'view', "bugID=$bugID"), 'parent'));
        }
    ...
    public function betaResolved($bugID)
    {
        ...
        if(!empty($_POST))
        {
            $this->bug->betaResolved($bugID);
            if(dao::isError()) die(js::error(dao::getError()));
            $actionID = $this->action->create('bug', $bugID, 'betaResolved', $this->post->comment);
            $this->sendmail($bugID, $actionID);
            die(js::locate($this->createLink('bug', 'view', "bugID=$bugID"), 'parent'));
        }
    ...
    public function sendmail($bugID, $actionID)
    {
        ...
        elseif(strtolower($toList) == 'closed' || $toList == 'alphaResolved' || $toList == 'betaResolved')
        {
