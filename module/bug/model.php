...
class bugModel extends model
{
    ...
    public function alphaResolved($bugID)
    {
        $oldBug = $this->getById($bugID);
        $now = helper::now();
        $bug = fixer::input('post')
            ->add('assignedDate',   $now)
            ->add('status',         'alphaResolved')
            ->add('lastEditedBy',   $this->app->user->account)
            ->add('lastEditedDate', $now)
            ->add('confirmed',      1)
            ->remove('comment')
    ...
    public function isClickable($object, $action)
    {
        ...
        if($action == strtolower('alphaResolved'))      return $object->status == 'resolved';
        if($action == strtolower('betaResolved'))      return $object->status == 'alphaResolved';
        if($action == 'close')      return $object->status == 'betaResolved';
    ...
    public function getUserResolvedBugs($account, $begin, $end)
    {
        $g = $this->loadModel('user')->getGroups($account);
        $str = 't1.resolvedBy';
        if(in_array('11',$g)){
            $str = 't1.closedBy';
        }
        $bugs = $this->dao->select('t1.*, t2.name projectName, t3.name productName')
            ->from(TABLE_BUG)->alias('t1')
            ...
            ->andWhere($str)->eq($account)->fi()
            ->andWhere('t1.closedDate')->ge($begin)->fi()
            ->andWhere('t1.closedDate')->le($end)->fi()
            ->orderBy('id desc')
            ->fetchAll();
    ...
    public function getUserUnclosedBugs($account)
    {
        $bugs = $this->dao->select('t1.*, t2.name projectName, t3.name productName')
            ->from(TABLE_BUG)->alias('t1')
            ->leftjoin(TABLE_PROJECT)->alias('t2')
            ...
            ->on('t1.product = t3.id')
            ->where('t1.deleted')->eq(0)
            ->andWhere('t1.status')->ne('closed')->fi()
