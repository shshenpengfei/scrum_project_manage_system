...
class story extends control
{
    ...
    public function view($storyID, $version = 0)
    {
        ...
        $cases        = $this->dao->select('id,title,openedBy')->from(TABLE_CASE)->where('story')->eq($storyID)->fetchAll();
    ...
    //需求延期数据导入 命令行执行 不分配权限
    public function storyDelayImport(){
        $this->story->delayImport();
    }

    //任务延期数据导入 命令行执行 不分配权限
    public function taskDelayImport(){
        $this->loadModel('task');
        $this->task->delayImport();
    }
