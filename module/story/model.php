...
class storyModel extends model
{
    ...
    public function getById($storyID, $version = 0, $setImgSize = false)
    {
        ...
        $story->projects = $this->dao->select('t1.project, t2.name, t2.status')
            ...
            ->fetchAll('project');
        $story->tasks     = $this->dao->select('id, name, assignedTo, project, status, consumed, `left`,deadline,estStarted,type')->from(TABLE_TASK)->where('story')->eq($storyID)->orderBy('id DESC')->fetchGroup('project');
    ...
    public function  subCreditByStoryId($storyid,$subvalue){
       // $this->dao->update(TABLE_STORY)
       //     ->set
       //     ->where('id')->eq($storyID)->exec();
    ...
    public function delayImport(){
        ...
        $stories = $this->dao->select('*')
            ->from(TABLE_STORY)
            ->where('stage')->ne('released')
            ->andWhere('deleted')->eq(0)
            ->andWhere('releasedDate')->le($today)
            ->andWhere('releasedDate')->ne('0000-00-00')
            ->fetchAll();

        foreach($stories as $s){
            $delayDays = floor((strtotime($today)-strtotime($s->releasedDate))/86400)+1;
            $data=(object)null;
            $delay = $this->dao->select('*')->from(TABLE_DELAYSTORY)->where('story')->eq($s->id)->fetch();
            if($delay){
                $data->delayDays = $delayDays;
                $this->dao->update(TABLE_DELAYSTORY)->data($data)
                ...
                $delayDate = $s->releasedDate;

                $tester  = $this->dao->select('assignedTo,finishedBy')->from(TABLE_TASK)
                    ->where('story')->eq($s->id)->andWhere('type')->eq('test')->fetchall();

                $QM = isset($tester[0]->finishedBy) && !empty($tester[0]->finishedBy) ?$tester[0]->finishedBy
                    :(isset($tester[0]->assignedTo)?$tester[0]->assignedTo:'');

                $product = $this->dao->select('*')->from(TABLE_PRODUCT)->where('id')->eq($s->product)->fetch();
                if($product){
                    $PO = $product->PO;
                }

                $projects = $this->dao->select('t1.project, t2.name, t2.status, t2.PO, t2.PM, t2.QM, t2.RM')
                    ->from(TABLE_PROJECTSTORY)->alias('t1')
                    ->leftJoin(TABLE_PROJECT)->alias('t2')
                    ->on('t1.project = t2.id')
                    ->where('t1.story')->eq($s->id)
                    ->orderBy('t1.project DESC')
                    ->fetchAll();
                if($projects){
                ...
                $this->dao->insert(TABLE_DELAYSTORY)->data($data)->exec();
            }
