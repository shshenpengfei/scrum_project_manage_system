...
<script type="text/javascript">
    link = createLink('story', 'ajaxGetUnFinishedFlag', 'storyID=' + '<?php echo $story->id;?>');
    $.get(link, function (flag) {
        if (flag == 'unfinished') {
            var len = $("#stage option").length;
            for (var i = len - 3; i < len; i++) {
                $("#stage option").eq(i).hide();
            }
        }
    });
    if($("#releasedDate").val() == '0000-00-00'){
        $("#releasedDate").val("");
    }

   $("#releasedDate").attr("readonly","readonly").css("width","50%")
