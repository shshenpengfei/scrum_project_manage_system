 ...
 * @package     project
 * @version     $Id: control.php 3330 2012-07-09 01:28:02Z chencongzhi520@gmail.com $
...
class project extends control
{
     ...
     * @return void
     */
    public function stand($projectID = 0, $status = 'all', $param = 0, $orderBy = '', $recTotal = 0, $recPerPage = 100, $pageID = 1)
    {
        $this->loadModel('tree');
        ...
        $this->config->project->search['actionURL'] = $this->createLink('project', 'stand', "projectID=$projectID&status=bySearch&param=myQueryID");
    ...
    public function story($projectID = 0, $orderBy = '')
    {
        ...
        if(!empty($stories)){
            $estimateSum = $this->task->getStoryTaskEstimateSum(array_keys($stories));
            $consumedSum = $this->task->getStoryTaskConsumedSum(array_keys($stories));
        }
        ...
        $this->view->estimateSum = $estimateSum;
        $this->view->consumedSum = $consumedSum;
        $this->view->pcwh = $pcwh;
    ...
    public function linkStory($projectID = 0, $browseType = '', $param = 0)
    {
        ...
        {
            $allStories = $this->story->getProductStories(array_keys($products), $moduleID = '0', $status = 'active', $orderBy = 'id_desc', $pager = null, $action = 'linkstory');
        }
     ...
     * @return void
     */
    public function sqlreview($projectID = 0, $orderBy = '')
    {
        /* Load these models. */
        $this->loadModel('story');
        $this->loadModel('user');
        $this->loadModel('task');
        $this->loadModel('sqlreview');
        ...
        $estimateSum = 0;
        $consumedSum = 0;
        $list    = $this->sqlreview->getSQLList($projectID, $orderBy);
        foreach($list as $k=>$v){
            $story = $this->story->getById($v->story);
            $list[$k]->story_title = $story->title;
        }
