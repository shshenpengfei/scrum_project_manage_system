...
<table class='cont-lt1'>
  <tr valign='top'>
    ...
    <?php endif;?>
    <td>
      <form method='post' action='<?php echo $this->createLink('task', 'batchStandChoose', "projectID=$project->id&from=projectTask&orderBy=$orderBy");?>'>
        <table class='table-1 fixed colored tablesorter datatable'>
          ...
          <tr class='colhead'>
            ...
            <th class='w-100px'>  <?php common::printOrderLink('standtime',  $orderBy, $vars, $lang->task->standmeeting);?></th>
          ...
          ?>
          <tbody>
          <?php $estimate_sum = 0;$consumed_sum = 0;?>
          ...
          <tr class='a-center'>
            <td>
                <?php if ($this->loadModel('task')->getStandDateByTaskid($task->id) == NULL) { ?>
            ...
            <td><?php echo $this->loadModel('task')->getStandDateByTaskid($task->id); ?></td>
          ...
          <?php $estimate_sum += $task->estimate;$consumed_sum += $task->consumed;?>
          <?php endforeach;?>
          </tbody>
          <tfoot>
            <tr>
              <?php $columns = $this->cookie->windowWidth > $this->config->wideSize ? 12 : 10;?>
              <td colspan='<?php echo $columns;?>'>
                <div class='f-left'>
                <?php
                if(common::hasPriv('task', 'batchEdit') and count($tasks))echo html::selectAll() . html::selectReverse();
                if(common::hasPriv('task', 'batchStandchoose') and count($tasks)) echo html::submitButton($lang->task->batchStandchoose);
