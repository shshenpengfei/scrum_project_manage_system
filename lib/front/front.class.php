...
class html
{
     ...
     * @param $end 结束时间
     */
    public  static  function calculateWorktime($begin,$end){

        $day_twelve_clock=date('Y-m-d',$begin)." 12:00:00";
        $day_twelve_clock_unix=strtotime($day_twelve_clock);

        $day_thirteen_clock=date('Y-m-d',$begin)." 13:00:00";
        $day_thirteen_clock_unix=strtotime($day_thirteen_clock);

        // <12,>13  -3600
        if($begin<=$day_twelve_clock_unix && $end>$day_thirteen_clock_unix){
            $lasttime=$end-$begin-3600;
        }
        // <12,<13
        else if($begin<=$day_twelve_clock_unix  && $end>$day_twelve_clock_unix && $end<$day_thirteen_clock_unix){
            $lasttime=$end-$begin-($end-$day_twelve_clock_unix);
        }
        // >12,>13
        else if($begin>$day_twelve_clock_unix && $begin<$day_thirteen_clock_unix && $end>$day_thirteen_clock_unix){
            $lasttime=$end-$begin-($day_thirteen_clock_unix-$begin);
        }
        //>12,<13
        else if($begin>$day_twelve_clock_unix  && $begin<$day_thirteen_clock_unix && $end>$day_twelve_clock_unix && $end<$day_thirteen_clock_unix){
            $lasttime=0;
        }
        else{
            $lasttime=$end?($end-$begin):0;
        }
        //echo $day_twelve_clock_unix."-".$day_thirteen_clock_unix;
        $day_twelve_clock="";
        $day_thirteen_clock="";
